...
export async function POST(request: Request) {
  try {
    ...
    logger.info(`Received general simulation request: ${prompt}`)

    try {
      // Call the backend API for simulation
      const data = await callBackendAPI(prompt, "general", parameters)
    ...
    logger.error("Error in general simulation:", error)
    return NextResponse.json(
      {
        success: false,

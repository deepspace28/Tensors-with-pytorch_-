...
export async function POST(request: Request) {
  try {
    ...
    logger.info(`Received physics simulation request: ${prompt}`)

    try {
      // Call the backend API for simulation
      const data = await callBackendAPI(prompt, "physics", parameters)
    ...
    logger.error("Error in physics simulation:", error)
    return NextResponse.json(
      {
        success: false,
